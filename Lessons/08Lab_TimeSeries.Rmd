...
Temp_Time <- ggplot(BR_complete, aes(x=BR_complete$Date,
                        y=BR_complete$Temperature_C)) +
  geom_line()+
...
Rain_Time <- ggplot(BR_complete, aes(x=Date,
                        y=Rainfall)) +
  geom_line()+
...
Brazil_ts <- ts(BR_complete$Temperature_C,
                start = c(1906,1), 
                frequency = 12)

Brazil2_ts <- ts(BR_complete$Rainfall,
                 start = c(1906,1), 
                 frequency = 12)
```
...
Brazil_Decomposed <- stl(Brazil_ts,
                         s.window = "periodic")
plot(Brazil_Decomposed)

Brazil2_Decomposed <- stl(Brazil2_ts, 
                          s.window = "periodic")
plot(Brazil2_Decomposed)
...
Brazil_Components <- as.data.frame(Brazil_Decomposed$time.series[,1:3])

Brazil_Components <- mutate(Brazil_Components,
        Observed = BR_complete$Temperature_C,     
        Date = BR_complete$Date)


Brazil2_Components <- as.data.frame(Brazil2_Decomposed$time.series[,1:3])

Brazil2_Components <- mutate(Brazil2_Components,
        Observed = BR_complete$Rainfall,     
        Date = BR_complete$Date)


# Exercise 10: Visualize how the trend maps onto the data for both series
ggplot(Brazil_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "coral1") +
  geom_hline(yintercept = 0, lty = 2) +
  ylab(expression("Temperature ºC"))

ggplot(Brazil2_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "firebrick") +
  geom_hline(yintercept = 0, lty = 2) +
