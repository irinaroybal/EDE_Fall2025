...
#day number that is in this range from 152 to 304
```
...
ex3 <- NTL.phys.data %>% filter(year4==1999)
# Exercise 2: 
...
> Answer: The headers are in columns, therefore it would be inefficient to filter using row numbers. We can't control the row numbers.
...
NTL.phys.data.TuesJul <- 
  filter(NTL.phys.data, lakename == "Tuesday Lake",
         year4 %in% c(1990:1999),
         format(NTL.phys.data$sampledate, "%m") == "07"
  )

  #or month(sampledate) == 7
...
NTL.phys.data.TuesJul <- 
  summarise(NTL.phys.data.TuesJul,  )
...
NTL.phys.data.irrad <- 
  pivot_longer(NTL.phys.data,
               ...
               irradianceDeck,
               cols_vary = "fastest",
               names_to = Irradiance_Type, Irradiance_Value)
